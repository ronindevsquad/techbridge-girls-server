...
module.exports = {
  show: function (req, res) {
    using(getConnection(), connection => {
      const query = "SELECT company, contact, email, picture, homepage, facebook, instagram, " +
        "linkedin, twitter FROM suppliers WHERE id = UNHEX(?) LIMIT 1";
      return connection.execute(query, [req.params.id]);
    })
      .spread(data => {
        if (data.length != 1)
          return res.status(400).json({ message: "Could not find supplier." });
  ...
  notifications: function (req, res) {
    Promise.join(using(getConnection(), connection => {
      const query = "SELECT u.id , o.proposals AS proposals, j.jobs AS jobs FROM suppliers u " +
  ...
  delete: function (req, res) {
    using(getConnection(), connection => {
      return connection.execute("DELETE FROM suppliers WHERE id = UNHEX(?) LIMIT 1", [req.user.id]);
    })
      .spread(data => {
  ...
  register: function (req, res) {
    if (req.body.type !== 1 || !req.body.company || !req.body.contact || !req.body.email
    ...
    // Valid supplier, encrypt password and save:
    bcrypt.genSaltAsync(10)
      .then(salt => {
      ...
      .then(hash => {
        return using(getConnection(), connection => {
          ...
          const query = "INSERT INTO suppliers SET id = UNHEX(?), company = ?, " +
    ...
    genSaltAsync(10)
      ...
      .then(() => {
        return using(getConnection(), connection => {
          ...
          req.body.email, hash];
          const query = "INSERT INTO suppliers SET id = UNHEX(?), company = ?, contact = ?, " +
