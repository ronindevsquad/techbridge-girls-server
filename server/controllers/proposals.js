		...
		jwt.verify(req.headers.authorization.split('Bearer ')[1], jwtKey, function (err, payload) {
			if (err)
				...
				Promise.join(using(getConnection(), connection => {
					...
					res.json({ proposals: proposals[0], reports: reprots[0] });
				})
					.catch(err => {
						console.log(err);
						res.status(400).json({ message: "Please contact an admin." });
	...
	uploadFiles: function (req, res) {
		if (!req.files || !req.files.buffer) {
			return res.status(400).json({
				message: "req.files does not exist"
			});
		}
		...
		jwt.verify(req.headers.authorization.split('Bearer ')[1], jwtKey, function (err, payload) {
			if (err) {
				...
				res.status(400).json({ message: "NDA must must be in .pdf format." });
			} else {
				let files = [];
				Promise.map(req.files, function (file) {
					...
					.catch(err => {
						console.log(err);
						res.status(400).json({ message: "Internal error, please contact an admin." });
					});
			}
		});
	},
	getProposalsForPage: function (req, res) {
		jwt.verify(req.headers.authorization.split('Bearer ')[1], jwtKey, function (err, payload) {
			if (err)
				...
				res.status(400).json({ message: "Only Suppliers may view all proposals." });
			else
				using(getConnection(), connection => {
		...
		jwt.verify(req.headers.authorization.split('Bearer ')[1], jwtKey, function (err, payload) {
			if (err)
				...
				Promise.join(using(getConnection(), connection => {
					...
					res.json({ files: files[0], offer: offer[0][0] });
				})
					.catch(err => {
						console.log(err);
						if (err.status)
	...
	create: function (req, res) {
		jwt.verify(req.headers.authorization.split('Bearer ')[1], jwtKey, function (err, payload) {
			if (err)
				...
				res.status(400).json({ message: "Only Makers may create proposals." });
				...
				res.status(400).json({ message: "You must specify a quantity of at least 1." });
			else if (req.body.audience != 0 && req.body.audience != 1)
				res.status(400).json({ message: "Invalid target suppliers provided." });
			else {
	...
	delete: function (req, res) {
		jwt.verify(req.headers.authorization.split('Bearer ')[1], jwtKey, function (err, payload) {
			if (err)
				...
				res.status(400).json({ message: "Only Makers may delete their proposals." });
			else {
				using(getConnection(), connection => {
