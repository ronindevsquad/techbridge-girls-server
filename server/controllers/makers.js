			...
			if (err)
				...
				using(getConnection(), connection => {
					const query = "SELECT company, contact, email, picture, homepage, facebook, instagram, " +
						"linkedin, twitter FROM makers LEFT JOIN urls ON makers.id = maker_id WHERE id = UNHEX(?) LIMIT 1";
			...
			if (err)
				...
				Promise.join(using(getConnection(), connection => {
					...
					if (payload.type == 0) // maker query
						// query = "select u.id , COUNT(p.id) as proposals from makers u " +
						// "LEFT OUTER JOIN proposals p on u.id = p.maker_id " +
						// "WHERE u.id = UNHEX(?) GROUP BY u.id";
						query = "SELECT u.id , p.proposals AS proposals, j.jobs AS jobs FROM makers u " +
						...
						// query = "select u.id , COUNT(o.maker_id) as proposals from makers u " +
						// "LEFT OUTER JOIN offers o on o.maker_id = u.id " +
						// "WHERE u.id = UNHEX(?) GROUP BY u.id"
						query = "SELECT u.id , o.proposals AS proposals, j.jobs AS jobs FROM makers u " +
							"LEFT OUTER JOIN (SELECT maker_id, COUNT(proposal_id) AS proposals FROM offers WHERE status > 0 GROUP BY maker_id) o " +
							"ON o.maker_id = u.id " +
							"LEFT OUTER JOIN (SELECT maker_id, COUNT(proposal_id) AS jobs FROM offers WHERE status = 2 GROUP BY maker_id) j " +
							"ON j.maker_id = u.id " +
							"WHERE u.id = UNHEX(?) GROUP BY u.id"
					return connection.execute(query, [req.params.id]);
				}), using(getConnection(), connection => {
					...
					if (payload.type == 0) //maker query
						...
						query = "SELECT COUNT(*) AS unread FROM messages WHERE proposal_id in " +
							"(SELECT proposal_id FROM offers WHERE maker_id = UNHEX(?)) " +
							"AND status = 0 AND maker_id != UNHEX(?)";
			...
			bcrypt.genSaltAsync(10)
				...
				.then(hash => {
					return using(getConnection(), connection => {
						...
						console.log(data)
						const query = "INSERT INTO makers SET id = UNHEX(?), type = ?, company = ?, " +
			...
			bcrypt.genSaltAsync(10)
				...
				.then(() => {
					return using(getConnection(), connection => {
						...
						req.body.email, hash];
						const query = "INSERT INTO makers SET id = UNHEX(?), type = ?, company = ?, contact = ?, " +
