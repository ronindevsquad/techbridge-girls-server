...
module.exports = {
	getOffersForProposal: function (req, res) {
		Promise.join(using(getConnection(), connection => {
			...
			for (let i = 0; i < offers[0].length; i++) {
				...
				if (offer.user_id != req.user.id && req.user.type != 0) {
	...
	index: function (req, res) {
		if (req.user.type !== 0)
			return res.status(401).json({ message: "Only Makers are allowed to view offers." });
	...
	show: function (req, res) {
		Promise.join(using(getConnection(), connection => {
			...
			return connection.execute(query, [req.params.proposal_id, req.params.user_id, req.user.id, req.user.id]);
		}), using(getConnection(), connection => {
	...
	create: function (req, res) {
		if (req.user.type !== 1)
			return res.status(401).json({ message: "Only Suppliers are allowed to create offers." });
		...
		using(getConnection(), connection => {
			...
			.spread((data) => {
				...
				return using(getConnection(), connection => {
					...
					return connection.execute(query, [req.body.proposal_id, req.user.id]);
				});
			})
			.then(() => {
				return res.end()
	...
	delete: function (req, res) {
		if (req.user.type !== 1)
			return res.status(401).json({ message: "Only Suppliers are allowed to delete their offers." });
	...
	send: function (req, res) {
		...
		// Validation done, insert into offers:
			...
			using(getConnection(), connection => {
				...
				req.body.total, req.body.completion, req.body.proposal_id, req.user.id, req.body.proposal_id];
					...
					return Promise.join(using(getConnection(), connection => {
						...
						for (let i = 0; i < req.body.materials.length; i++) {
							...
							data.push(["UNHEX(REPLACE(UUID(), '-', ''))", material.material, material.weight,
								material.cost, "NOW()", "NOW()", `UNHEX('${req.body.proposal_id}')`, `UNHEX('${req.user.id}')`]);
						}
						...
						for (let i = 0; i < req.body.machines.length; i++) {
							...
							data.push(["UNHEX(REPLACE(UUID(), '-', ''))", 0, machine.labor, machine.time,
								...
								`UNHEX('${req.body.proposal_id}')`, `UNHEX('${req.user.id}')`]);
						}
	...
	accept: function (req, res) {
		if (req.user.type !== 0)
			return res.status(400).json({ message: "Only Makers are allowed to accept offers." });

		if (!req.body.proposal_id || !req.body.user_id)
